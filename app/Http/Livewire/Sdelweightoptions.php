...
class Sdelweightoptions extends Component
{
    use WithPagination;
    ...
    {
        $this->validate();
        Weightoption::create($this->modeldata2());
    ...
    {
        return Weightoption::onlyTrashed()->paginate(5);
    }
    /**
     * The update function.
     *
    ...
    {
        $this->validate();
        Weightoption::find($this->modelId2)->update($this->modeldata2());
        $this->modal2FormVisible = false;
        $this->dispatchBrowserEvent('event-notification', [
    ...
    {
        Weightoption::onlyTrashed()->find($this->modelId2)->forceDelete($this->modeldata2());
    ...
    {
        Weightoption::withTrashed()->find($this->modelId2)->restore();
        $this->modal2ConfirmRestoreVisible = false;
        $this->resetPage();
        $this->dispatchBrowserEvent('event-notification', [
            'eventName' => 'Restored Page',
            'eventMessage' => 'The Weightoption (' . $this->modelId2 . ') has been restored!',
        ]);
    }
    ...
    {
        $data2 = Weightoption::withTrashed()->find($this->modelId2);
    ...
    {
        return view('livewire.sdelweightoptions', [
            'data2' => $this->read(),
        ]);
    }
}
